...
bsh = BoozerAnalytic(stel.etabar, stel.B0, stel.iota-stel.iotaN, stel.G0, stel.B0*r_max*r_max/2, stel.iota)

Ekin = energy*ONE_EV
m=mass*PROTON_MASS
q=charge*ELEMENTARY_CHARGE
stz_inits = np.array([[stel.B0*r_initial*r_initial/2, theta_initial, phi_initial]])
...
rpos_cylindrical_neat = np.array(
    stel.to_RZ(
        np.array([r_pos_neat, theta_pos_neat, varphi_pos_neat]).transpose()
    )
)
rpos_cartesian_neat = np.array(
    [
        rpos_cylindrical_neat[0] * np.cos(rpos_cylindrical_neat[2]),
        rpos_cylindrical_neat[0] * np.sin(rpos_cylindrical_neat[2]),
        rpos_cylindrical_neat[1],
    ]
)
...
plot_orbit3d(
    boundary=boundary,
    rpos_cartesian=rpos_cartesian_neat,
    distance=6,
    show=False,
)
plot_orbit3d(
