...
diff_phi = (
    np.unwrap(np.mod(g_orbit.rpos_cylindrical[2], 2 * np.pi))
    - np.unwrap(np.mod(g_orbit_vmec.rpos_cylindrical[2], 2 * np.pi))
) / (2 * np.pi)
...
plt.plot(
    g_orbit.time * 1e6,
    np.mod(g_orbit.rpos_cylindrical[2], 2 * np.pi),
    label="Particle 1",
)
plt.xlabel(r"t  ($\mu$s)")
plt.ylabel(r"$\Phi$")
...
plt.subplot(3, 4, 7)
plt.plot(
    g_orbit_vmec.time * 1e6,
    np.mod(g_orbit_vmec.rpos_cylindrical[2], 2 * np.pi),
    label="Particle 1",
)
plt.xlabel(r"t  ($\mu$s)")
plt.ylabel(r"$\Phi_V $")
