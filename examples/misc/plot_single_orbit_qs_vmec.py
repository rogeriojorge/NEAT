...
OUT_DIR=os.path.join(Path(__file__).parent.resolve(),f'comparison_qsc_vmec_orbits_s_b={s_boundary}')
os.makedirs(OUT_DIR, exist_ok=True)
minor_radius_plot=r_avg*np.sqrt(s_boundary)
filename_folder = os.path.join(OUT_DIR, filename)
g_field.to_vmec(filename=filename_folder,r=minor_radius_plot, params={"ntor":7, "mpol":7, "niter_array":[2000,2000,20000]}, ntheta=14, ntorMax=7) #standard ntheta=20, ntorMax=14
# if mayavi_loaded:
...
g_particle = ChargedParticle(r_initial=r_avg*np.sqrt(s_initial),\
                             theta_initial=theta_initial, phi_initial=phi_initial, energy=energy,\
                                Lambda=Lambda, charge=charge, mass=mass, vpp_sign=vpp_sign)

phi_VMEC=g_field.to_RZ([[r_avg*np.sqrt(s_initial),\
                         theta_initial,phi_initial]])[2][0]
    
g_particle_vmec = ChargedParticle(r_initial=s_initial, theta_initial=np.pi - theta_initial, \
                                  phi_initial=phi_VMEC, energy=energy, Lambda=Lambda, charge=charge,\
                                      mass=mass, vpp_sign=vpp_sign)
...
g_orbit_vmec =  ParticleOrbit(g_particle_vmec, g_field_vmec, nsamples = nsamples, tfinal = tfinal, constant_b20 = constant_b20)
total_time_vmec = time.time() - start_time_vmec
print(f"Finished in {total_time_vmec}s")

print("Creating parameter plot - NA")
g_orbit.plot(show=False, savefig='comparison_qsc_vmec_orbits_s_b=' + str(s_boundary) + '/s_b=' + str(s_boundary) + '_lambda=' + str(Lambda) + '_s_i=' + str(s_initial) + \
    '_theta=' + str(theta_initial) + '_phi=' + str(phi_initial) + '_neat_parameters.png')

print("Creating 3D plot - NA")
g_orbit.plot_orbit_3d(show=False, r_surface=r_avg*np.sqrt(s_boundary),savefig='comparison_qsc_vmec_orbits_s_b=' + str(s_boundary) + '/s_b=' + str(s_boundary) + '_lambda=' + str(Lambda) + '_s_i=' + str(s_initial) + \
    '_theta=' + str(theta_initial) + '_phi=' + str(phi_initial) + '_3d_neat.pdf')

print("Creating B contour plot - NA")
g_orbit.plot_orbit_contourB(show=False)

print("Creating parameter plot - VMEC")
g_orbit_vmec.plot(show=False, savefig='comparison_qsc_vmec_orbits_s_b=' + str(s_boundary) + '/s_b=' + str(s_boundary) + '_lambda=' + str(Lambda) + '_s_i=' + str(s_initial) + \
    '_theta=' + str(theta_initial) + '_phi=' + str(phi_initial) + '_vmec_parameters.png')

print("Creating 3D plot - VMEC")
g_orbit_vmec.plot_orbit_3d(show=False, savefig='comparison_qsc_vmec_orbits_s_b=' + str(s_boundary) + '/s_b=' + str(s_boundary) + '_lambda=' + str(Lambda) + '_s_i=' + str(s_initial) + \
    '_theta=' + str(theta_initial) + '_phi=' + str(phi_initial) + '_3d_vmec.pdf')
...
g_orbit.plot_diff_cyl(g_orbit_vmec, show=False, savefig='comparison_qsc_vmec_orbits_s_b=' + str(s_boundary) + '/s_b=' + str(s_boundary) + '_lambda=' + str(Lambda) + '_s_i=' + str(s_initial) + '_theta=' + str(theta_initial) + '_phi=' + str(phi_initial) + '_cyl.png')

print("Calculating differences between near axis and vmec - Boozer")
g_orbit.plot_diff_boozer(g_orbit_vmec, r_minor=(r_avg*np.sqrt(s_boundary)), MHD_code='VMEC', show=False, savefig='comparison_qsc_vmec_orbits_s_b=' + str(s_boundary) + '/s_b=' + str(s_boundary) + '_lambda=' + str(Lambda) + '_s_i=' + str(s_initial) + '_theta=' + str(theta_initial) + '_phi=' + str(phi_initial) + '_boozer.png')
