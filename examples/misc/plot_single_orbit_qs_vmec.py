...
wout_filename = f"wout_nearaxis_sboundary{s_boundary}.nc"

g_field = StellnaQS.from_paper(1, B0=B0, nphi=201)
g_field.to_vmec(filename=filename, r=0.1*np.sqrt(s_boundary))
subprocess.run([f"{os.path.join(os.path.dirname(__file__))}./xvmec2000", filename])
g_field_vmec = Vmec(wout_filename=wout_filename)
print(g_field.to_RZ([[0.1*np.sqrt(s_initial),theta_initial,phi_initial]]))
