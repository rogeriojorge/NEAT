...
import numpy as np
from neat.fields import StellnaQS, Simple
from neat.tracing import ChargedParticleEnsemble, ParticleEnsembleOrbit, ParticleEnsembleOrbit_Simple
...
g_field = StellnaQS(rc=g_field_basis.rc*Rmajor_ARIES, zs=g_field_basis.zs*Rmajor_ARIES, etabar=g_field_basis.etabar/Rmajor_ARIES, B2c=g_field_basis.B2c*(B0/Rmajor_ARIES/Rmajor_ARIES),\
                                B0=B0, nfp=g_field_basis.nfp, order='r3', nphi=131)
...
print(f"Final loss fraction with jacobian weights and B20 constant = {g_orbits.loss_fraction_array[-1]*100}%")
...
vmec.run()
g_field_simple = Simple(wout_filename=wout_filename, B_scale=B0, Aminor_scale=Aminor_scale)

g_particle_simple = ChargedParticleEnsemble(r_initial=s_initial, energy=energy, charge=charge, mass=mass)

print("Starting particle tracer")
start_time = time.time()

g_orbits_simple = ParticleEnsembleOrbit_Simple(g_particle_simple, g_field_simple, tfinal=tfinal, nthreads=nthreads_array[0], \
    nparticles=nparticles, nsamples=nsamples, notrace_passing=notrace_passing)
