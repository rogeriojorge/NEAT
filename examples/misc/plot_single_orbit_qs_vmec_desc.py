...
OUT_DIR=os.path.join(Path(__file__).parent.resolve(),f'comparison_qsc_desc_orbits_s_b={s_boundary}')
os.makedirs(OUT_DIR, exist_ok=True)
filename_folder = os.path.join(OUT_DIR, filename)

# Creating a DESC object from pyQsc near acis
g_field_desc = Equilibrium.from_near_axis(g_field, # the Qsc equilibrium object 
...
phi_initial_vmec=g_field.to_RZ([[r_avg*np.sqrt(s_initial),theta_initial,phi_initial]])[2][0]

g_particle_vmec = ChargedParticle(r_initial=s_initial, theta_initial= - theta_initial, \
                                  phi_initial=phi_initial_vmec, energy=energy, Lambda=Lambda, \
                                    charge=charge, mass=mass, vpp_sign=vpp_sign)
...
g_orbit.plot(show=False, savefig='comparison_qsc_desc_orbits_s_b=' + str(s_boundary) + '/s_b=' + str(s_boundary) + '_lambda=' + str(Lambda) + '_s_i=' + str(s_initial) + \
...
g_orbit.plot_orbit_3d(show=False, r_surface=r_avg*np.sqrt(s_boundary),savefig='comparison_qsc_desc_orbits_s_b=' + str(s_boundary) + '/s_b=' + str(s_boundary) + '_lambda=' + str(Lambda) + '_s_i=' + str(s_initial) + \
...
g_orbit_vmec.plot(show=False, savefig='comparison_qsc_desc_orbits_s_b=' + str(s_boundary) + '/s_b=' + str(s_boundary) + '_lambda=' + str(Lambda) + '_s_i=' + str(s_initial) + \
    '_theta=' + str(theta_initial) + '_phi=' + str(phi_initial) + '_desc_parameters.png')

# print("Creating 2D plot")
# g_orbit.plot_orbit(show=False)

# print("Creating 3D plot - VMEC")
g_orbit_vmec.plot_orbit_3d(show=False, savefig='comparison_qsc_desc_orbits_s_b=' + str(s_boundary) + '/s_b=' + str(s_boundary) + '_lambda=' + str(Lambda) + '_s_i=' + str(s_initial) + \
    '_theta=' + str(theta_initial) + '_phi=' + str(phi_initial) + '_3d_desc.pdf')
...
g_orbit.plot_diff_cyl(g_orbit_vmec, show=False, savefig='comparison_qsc_desc_orbits_s_b=' + str(s_boundary) + '/s_b=' + str(s_boundary) + '_lambda=' + str(Lambda) + '_s_i=' + str(s_initial) + '_theta=' + str(theta_initial) + '_phi=' + str(phi_initial) + '_cyl.png')

print("Calculating differences between near axis and vmec - Boozer")
g_orbit.plot_diff_boozer(g_orbit_vmec, r_minor=(r_avg*np.sqrt(s_boundary)), MHD_code='DESC', show=False, savefig='comparison_qsc_desc_orbits_s_b=' + str(s_boundary) + '/s_b=' + str(s_boundary) + '_lambda=' + str(Lambda) + '_s_i=' + str(s_initial) + '_theta=' + str(theta_initial) + '_phi=' + str(phi_initial) + '_boozer.png')
