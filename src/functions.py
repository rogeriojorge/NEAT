...
def orbit(stel, params, B20real):
    ...
    Valfven = 1/np.sqrt(mu0*params['rhom']*m_proton*1.e+19)

    # Calculate canonical angular momentum p_phi
    m_proton = 1.67262192369e-27
    e = 1.602176634e-19
    mu0 = 1.25663706212e-6
    Valfven = stel.Bbar/np.sqrt(mu0*params['rhom']*m_proton*1.e+19)
    p_phi1=params['mass']*m_proton*v_parallel*Valfven*(stel.G0+r_pos**2*(stel.G2+(stel.iota-stel.iotaN)*stel.I2))/Bfield/stel.Bbar
    p_phi2=params['charge']*e*r_pos**2*stel.Bbar/2*stel.iotaN
    p_phi=p_phi1-p_phi2

    return [time,r_pos,theta_pos,phi_pos,total_energy,params['theta0'],params['phi0'],params['Lambda'],params['energy'],p_phi,rdot,thetadot,phidot,vppdot,v_parallel,Bfield,varphi]
...
def create_BEAMS3D_input(name,stel,rhom,mass,r0,
            theta0,phi0,charge,Tfinal,rParticleVec,
            nsamples,zParticleVec,phiParticleVec,result):
