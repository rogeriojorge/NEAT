...
etabar=0.9
B0 = 3
stel = Qsc(rc=rc, zs=zs, etabar=etabar, nfp=nfp, I2=lambda_current*B0/2, nphi=nphi, B0=B0)
# B0 = 3
# r0=0.15
# lambda_current = 0.0
# Lambda   = [0.1]
...
phi2c = (B0/2)*(  2*np.pi*B0*F0*stel.sigma-np.matmul(stel.d_d_phi,stel.curvature)/(stel.curvature*stel.d_l_d_phi) )
phi2s = (B0/2)*(-(2*np.pi*B0*F0*stel.etabar*stel.etabar/(stel.curvature*stel.curvature)+stel.torsion) )
...
def orbit(stel,r0,theta0,phi0,charge,rhom,mass,Lambda,energy,nsamples,Tfinal):
    ...
    sol = np.array(
        ...
        (
            int(stel.nfp), stel.Bbar, lambda_current, np.append(s_of_phi,stel.axis_length),
            np.append(stel.curvature, stel.curvature[0]), np.append(stel.torsion, stel.torsion[0]),
            [B0]*nphi, np.append(phi2c, phi2c[0]), np.append(phi2s, phi2s[0]),
...
if __name__ == '__main__':
        ...
        for _Lambda in Lambda:
            for _energy in energy:
                for _theta0 in theta0:
                    orbit_temp=orbit(stel,r0,_theta0,_phi0,charge,rhom,mass,_Lambda,_energy,nsamples,Tfinal)
    ...
    if showPlots==1:
        ...
        rpos_cylindrical = [[stel.R0_func(res[3]),0*res[3],stel.Z0_func(res[3])]
                            +res[1]*(np.cos(res[2])*[stel.normal_R_spline(res[3]),  stel.normal_phi_spline(res[3]),  stel.normal_z_spline(res[3])]
                                    +np.sin(res[2])*[stel.binormal_R_spline(res[3]),stel.binormal_phi_spline(res[3]),stel.binormal_z_spline(res[3])])
                            for res in result]
        rpos_cartesian = [[rpos_cylindrical[i][0]*np.cos(result[i][3])-rpos_cylindrical[i][1]*np.sin(result[i][3]),rpos_cylindrical[i][0]*np.sin(result[i][3])+rpos_cylindrical[i][1]*np.cos(result[i][3]),rpos_cylindrical[i][2]] for i in range(len(result))]
        boundary = np.array(stel.get_boundary(r=0.9*r0,nphi=90,ntheta=25,ntheta_fourier=16,mpol=8,ntor=15))

        plt.subplot(3, 3, 8)
        [plt.plot(rpos_cylindrical[i][0],rpos_cylindrical[i][2]) for i in range(len(result))]
        ...
        [plt.plot(res[0],B0/(1+res[1]*curvature_spline(res[3])*np.cos(res[2])),label='theory') for res in result]
