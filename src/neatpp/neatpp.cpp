...
public:
    ...
    const std::vector<Gyron>& gyron() const {
        return gyron_;
    };
private:
...
std::vector< std::vector<double>> gc_solver_qs_partial_ensemble(
                                   double G0, double G2, double I2, double nfp, double iota,
                                   double iotaN, const std::vector<double>& phi_grid,
                                   double B0, double B1c, std::vector<double>& B20, double B2c,
...
//   steffen_gsl_factory ifactory;
    ...
    double B_min = std::max( 0.01, abs(B0) - abs(r_max * B1c) - r_max * r_max * (B20_max + abs(B2c)) );
