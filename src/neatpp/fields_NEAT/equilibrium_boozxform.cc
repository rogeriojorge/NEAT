...
equilibrium_boozxform::equilibrium_boozxform(
    ...
    ixm_b_(g->parser()->ixm_b()), ixn_b_(g->parser()->ixn_b()), iota_b_(nullptr), G_(nullptr), I_(nullptr) {
  ...
  std::valarray<double> iota_i = (p->iota_b()) / this->m_factor();
...
IR3 equilibrium_boozxform::contravariant(const IR3& position, double time) const {
  ...
  double Bv = (1/jac);
  // cout << "contra: " << (this->m_factor()) << endl;
  return {0.0, (*iota_b_)(s)*Bv, Bv/ ((this->m_factor()))};
}
...
dIR3 equilibrium_boozxform::del_contravariant(
  ...
  // cout << "del_contra: " << (this->m_factor()) << endl;
  return {
      0.0, 0.0, 0.0,
...
dIR3 equilibrium_boozxform::del_covariant(
  ...
  // cout << "del_covar: " << (*G_)(s) << " " << d_w_Bv << endl;
