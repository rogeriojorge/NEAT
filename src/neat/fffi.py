...
class FortranLibrary:
    def __init__(self, name, maxdim=7, path=None, compiler=None):
        ...
        if libfile is None:
            raise RuntimeError(f"Cannot find library {name} with extensions {libexts}")

        if self.compiler is None:
            libstrings = subprocess.check_output(["strings", libfile])
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(
            """Fortran library \'{}\' has no routine
                                \'{}\'.""".format(
                self.name, attr
            )
        )
    ...
    def compile(
        self, tmpdir=".", verbose=0, debugflag=None, skiplib=False, extra_objects=None
    ):
        """
    ...
    def new(self, typename, value=None):
        ...
        raise NotImplementedError("Cannot assign value to type {}".format(typename))
...
class FortranModule:
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(
            """Fortran module \'{}\' has no attribute
                                \'{}\'.""".format(
                self.name, attr
            )
        )
    ...
    def cdef(self, csource):
        ...
        # GNU specific
        if self.lib.compiler["name"] == "gfortran":
            self.csource += csource.format(mod="__" + self.name + "_MOD", suffix="")
        elif self.lib.compiler["name"] == "ifort":
            self.csource += csource.format(mod=self.name + "_mp", suffix="_")
        else:
            raise NotImplementedError(
                """Compiler {} not supported. Use gfortran or ifort
                """.format(
...
dtypemap = {1: "integer", 2: "logical", 3: "real", 4: "complex"}
...
def ccodegen_sub(subprogram, module=True):
    ...
    for arg in subprogram.args:
        ...
        if dtype.startswith("type"):
            ...
            if (
                shape is None or shape[0] is None or shape[0][1] is None
            ):  # Assumed size array
...
class InternalSubprogram(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.declarations = [
            stmt for stmt in self.statements if isinstance(stmt, Declaration)
        ]
