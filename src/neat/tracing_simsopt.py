...
def trace_particles(bfield, label, mode="gc_vac"):
    ...
    gc_tys, gc_phi_hits = trace_particles_starting_on_curve(
        ...
        phis=phis,
        tol=1e-9,
        stopping_criteria=[LevelsetStoppingCriterion(sc_particle.dist)],
        mode=mode,
        forget_exact_path=True,
    )
    t2 = time.time()
    print(
        f"Time for particle tracing={t2-t1:.3f}s. Num steps={sum([len(l) for l in gc_tys])//nparticles}",
        flush=True,
    )
    if comm is None or comm.rank == 0:
        ...
        plot_poincare_data(
            gc_phi_hits,
            phis,
            OUT_DIR + f"poincare_particle_{label}_loss.png",
            mark_lost=True,
        )
        plot_poincare_data(
            gc_phi_hits,
            phis,
            OUT_DIR + f"poincare_particle_{label}.png",
            mark_lost=False,
        )
